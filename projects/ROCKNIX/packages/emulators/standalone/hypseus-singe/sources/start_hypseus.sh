...
if [ -f "${config}/hypinput.override.ini" ] || [ -f "${config}/hypinput.${name}.override.ini" ]; then
    ...
    for file in /usr/share/libretro/autoconfig/*.cfg; do
        ...
        if [ "${JOYNAME}" == "${GAMEPAD}" ]; then
            ...
            sed -i "1 c\# No override found... ${name} ... autoconfig" ${configfile}
        fi
