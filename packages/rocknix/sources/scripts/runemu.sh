...
function loginit() {
  if [ ${LOG} == true ]; then
    if [ -e ${LOG_DIRECTORY}/${LOG_FILE} ]; then
...
if [ ! -z "${FORCEPACK}" ] && [ "${FORCEPACK}" = "On" ]; then
